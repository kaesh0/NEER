...
export default function MarineInteractiveMap({ height = '100%', className = '', focusPoint = null, onNavigate, setExploredLocation, exploredLocation, compact = false }) {
  const { t } = useTranslation()
  ...
  return (
    <div className={`relative rounded-[0.875rem] overflow-hidden border border-neer-border z-0 ${compact ? 'pointer-events-none' : ''} ${className}`} style={{ height: height === '100%' ? '100%' : height, minHeight: compact ? 0 : '400px', width: '100%' }}>
      <MapContainer
