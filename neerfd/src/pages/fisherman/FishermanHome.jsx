...
function ZoneCard({ zone, onNavigate, origin }) {
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            <button
              onClick={() => {
                // Zones are relative to the user's (live) location, never a
                // hardcoded city — prefer the API geometry, then origin.
                const zLat = zone.lat || origin?.lat
                const zLng = zone.lng || origin?.lng
                onNavigate && zLat != null && zLng != null && onNavigate('map', { type: 'zone', id: zone.id, lat: zLat, lng: zLng })
              }}
      ...
      <div className={`relative z-10 w-full transition-opacity duration-300`}>
        ...
        <section className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3 text-neer-sm text-neer-ink-secondary">
            ...
            <span className="font-medium">{t(location.name)}</span>
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {fishingZones.map((zone) => (
              <ZoneCard key={zone.id} zone={zone} onNavigate={onNavigate} origin={{ lat: location.lat, lng: location.lng }} />
            ))}
            {pfzRecommendation && (
