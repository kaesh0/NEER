...
import MapAmbience from '../../components/fisherman/MapAmbience.jsx'
import { getMapData, getConditions, getDecision } from '../../data/mock/fishermanData.js'
...
export default function FishermanMap({ data, loading, error, onRetry, focusPoint, setFocusPoint, onNavigate, setExploredLocation, exploredLocation }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/analysis?persona=fisherman&lat=${exploredLocation.lat}&lng=${exploredLocation.lng}`)
      .then(async (res) => {
        if (!res.ok) {
    ...
    <div className="relative flex flex-col h-[calc(100vh-4.5rem)] pb-4 md:pb-8 max-w-[1440px] mx-auto w-full px-4 pt-6">
            ...
            <Card variant="bordered" className="bg-white p-6 md:p-8 shadow-neer-lg rounded-2xl border-slate-200">
              ...
              {locLoading ? (
                ...
                <div className="py-4 space-y-4">
                  <StructuredLocationCard locData={locData} persona="fisherman" />
      ...
      <div className="relative z-10 flex flex-col h-full w-full">
        ...
        <div className="flex-1 w-full mt-4 min-h-[400px]">
          <InteractiveMap data={data} height="100%" focusPoint={focusPoint} onNavigate={onNavigate} setExploredLocation={setExploredLocation} exploredLocation={exploredLocation} />
        </div>
      </div>
    </div>
  )
