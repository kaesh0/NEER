...
import { useHomeLocation } from './LocationContext.jsx'

const AuthContext = createContext(null)

export function AuthProvider({ children }) {
  // Home location lives in LocationContext; auth pages push the location the
  // user picked (or the one restored from their backend profile) into it.
  ...
  const login = async (emailOrMobile, password, persona, location) => {
    ...
    try {
      const res = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({ emailOrMobile: cleanId, password, persona: chosenRole, location })
      })
      if (res.ok) {
    ...
    if (backendUser) {
      setUser(backendUser)
      setIsGuest(false)
      const effective = location || backendUser.location
      if (effective?.name && Number.isFinite(effective.lat)) setHomeLocation(effective)
      return backendUser
    }
    ...
    setIsGuest(false)
    if (simulatedUser.location) setHomeLocation(simulatedUser.location)
  ...
  const register = async (details) => {
    ...
    const location = details.location && details.location.name ? details.location : null
    ...
    if (backendUser) {
      setUser(backendUser)
      setIsGuest(false)
      if (location) setHomeLocation(location)
      return backendUser
    }
